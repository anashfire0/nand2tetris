...
class CodeWriter:
    ...
    def offset_for_temp(self, offset, register='R15'):
        asm_commands = ['@5', 'D=M', f'@{offset}', 'D=D+A', f'@{register}', 'M=D']
        self.asm_file.writelines(command+'\n' for command in asm_commands)
    ...
    def return_to_caller(self):
        ...
        asm_commands = ['@5', 'D=A', '@R14', 'M=D', '@R13', 'D=M', '@R14', 'D=D-M', 'A=D', 'A=M', '0;JMP']
        self.asm_file.writelines(command+'\n' for command in asm_commands)
    ...
    def reposition_pointer(self, pointer):
        ...
        if pointer == 'SP':
            ...
            asm_commands = [f'@{offset.get(pointer)}', 'D=A', '@R14', 'M=D', '@R13', 'D=M', '@R14', 'D=D-M', 'A=D', 'D=M', f'@{pointer}', 'M=D']
