...
class Tokenizer:
    ...
    def __init__(self, input_file):
        ...
        self.void_subroutines = ['printInt', 'printOutput', 'poke', 'printString']
        # self.identify_void_subroutines()
        ...
        with open(self.input_file_name, 'r') as input_file:
            for line in input_file:
                ...
                for possible_token in Tokenizer.split_symbols(line):
                    if possible_token.startswith('"') and not possible_token.endswith('"'):
                        tmp_str += possible_token
                        continue
                    if tmp_str:
                        if possible_token.endswith('"'):
    ...
    def identify_void_subroutines(self):
        for index, token in enumerate(self.all_tokens):
            token_value, token_type = token
            if token_type == 'identifier' and self.all_tokens[index+1][0] == '(' \
                and self.all_tokens[index-1][0] == 'void':
                self.void_subroutines.append(token_value)
